...
WITH function() {
  RETURN sqrt(sum((a.NB - b.NB)^2 for a in reviews1
               for b in reviews2
               where a.areaId = b.areaId))
} AS euclideanDistance,
// Function to calculate Cosine similarity
function() {
  RETURN sum(a.NB * b.NB for a in reviews1
             for b in reviews2
             where a.areaId = b.areaId) /
         (sqrt(sum(a.NB^2 for a in reviews1)) *
          sqrt(sum(b.NB^2 for b in reviews2)))
} AS cosineSimilarity
...
WITH u1, u2, 
     COLLECT({areaId: a.gid, NB: r1.NB}) AS reviews1,
     COLLECT({areaId: a.gid, NB: r2.NB}) AS reviews2,
